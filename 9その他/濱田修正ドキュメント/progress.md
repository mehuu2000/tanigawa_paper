...
* `evaluation_experiment.ipynb` の正例入力だけを誤植なし版へ切り替え、負例は従来データのまま据え置いて評価した。
* 使用した誤植なし正例データは以下の 3 ファイルである。
  * `positive_reference_jsai_eval_none_typo.txt`
  * `positive_reference_ipsj_eval_none_typo.txt`
  * `positive_reference_lsj_eval_none_typo.txt`
* DOI 対応は従来どおり同じ 863 件の `doi_list.txt` を用いた。
* 実行ログ上では IPSJ / JSAI の使用ファイル名に入れ違いの記述があったため、以下ではスタイル名ベースで結果を整理する。

#### 正例 IPSJ（誤植なし）

* 実行日時: `6/2 1:00`
* 総実行セル: `Cell 4, 15, 24, 35, 41, 46`
* BM25（Cell 15）: `854 / 8 / 1 / 0`
* BM25 長さ補正（Cell 24）: `846 / 16 / 1 / 0`
* RC（Cell 35）: `863 / 0 / 0 / 0`
* CC（Cell 41）: `863 / 0 / 0 / 0`
* MC（Cell 46）: `863 / 0 / 0 / 0`

#### 正例 JSAI（誤植なし）

* 実行日時: `6/2 1:00`
* 総実行セル: `Cell 4, 15, 24, 35, 41, 46`
* BM25（Cell 15）: `846 / 17 / 0 / 0`
* BM25 長さ補正（Cell 24）: `835 / 28 / 0 / 0`
* RC（Cell 35）: `863 / 0 / 0 / 0`
* CC（Cell 41）: `863 / 0 / 0 / 0`
* MC（Cell 46）: `863 / 0 / 0 / 0`
...
* 総実行セル: `Cell 4, 15, 24, 35, 41, 46`
* BM25（Cell 15）: `854 / 8 / 1 / 0`
* BM25 長さ補正（Cell 24）: `862 / 0 / 1 / 0`
* RC（Cell 35）: `863 / 0 / 0 / 0`
* CC（Cell 41）: `862 / 1 / 0 / 0`
* MC（Cell 46）: `863 / 0 / 0 / 0`

#### 誤植なし正例 3 スタイル合計

* BM25: `2554 / 33 / 2 / 0`
* BM25 長さ補正: `2543 / 44 / 2 / 0`
* RC: `2589 / 0 / 0 / 0`
* CC: `2588 / 1 / 0 / 0`
* MC: `2589 / 0 / 0 / 0`
...
* 誤植なし正例に切り替えたことで、RC と MC は 3 スタイルすべてで `863 / 0 / 0 / 0` となり、全件同定できた。
* CC も IPSJ / JSAI では全件同定となり、LSJ でのみ `case2 = 1` が残った。
* BM25 系も全体として改善したが、RC / CC / MC ほど完全には揃わず、依然として見逃しや誤同定が残った。
* LSJ の CC で 1 件だけ `case2` が残ったのは、候補 DOI 自体は正しいが `sim_p_score` が既存閾値 `0.8730` をわずかに下回った境界事例である可能性が高い。
* このことから、CC の閾値は誤植ありデータで最適化された値のままであり、誤植なし正例に対してもほぼ有効だが、境界事例が 1 件だけ残ることが分かった。

#### Step4 時点の結論

* 誤植なし正例に切り替えたことで、RC / MC は完全一致、CC も 1 件を除いて完全一致となった。
* 誤植の影響は BM25 系よりも被覆率ベースの RC / CC / MC でより明確に除去され、特に RC / MC は全件同定まで改善した。
* 次の段階では、誤植あり / 誤植なしの差分を比較し、どの手法が誤植の影響を受けやすいかを整理する価値がある。
